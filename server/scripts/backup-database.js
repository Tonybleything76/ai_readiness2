 ...
 * 
 * Usage:
 *   node server/scripts/backup-database.js [options]
 * 
 * Options:
 *   --output-dir <path>   Backup output directory (default: ./backups)
 *   --compress            Compress backup with gzip
 *   --retain-days <days>  Days to retain old backups (default: 30)
 *   --format <format>     Backup format: sql, custom, tar (default: sql)
 *   --verbose             Enable verbose logging
 * 
 * Environment Variables:
 *   DATABASE_URL         PostgreSQL connection string (required)
 *   BACKUP_OUTPUT_DIR    Default backup directory
 *   BACKUP_RETAIN_DAYS   Default retention period
 * 
 * Cron Examples:
...
function parseDatabaseUrl(databaseUrl) {
  try {
    ...
    return {
      ...
      ssl: sslmode && ['require', 'verify-full', 'verify-ca'].includes(sslmode)
    };
  } catch (error) {
...
async function createBackup(config) {
  ...
  // Only add verbose flag if explicitly requested
  if (config.verbose) {
  ...
  try {
    ...
    if (config.format === 'sql' && !config.compress) {
      // For plain format without compression, use --file option
      ...
      pgDumpProcess = spawn('pg_dump', pgDumpArgs, { env, shell: false });
      ...
      if (config.compress) {
        ...
        // Direct file output
        const outputStream = createWriteStream(backupPath);
        await pipeline(pgDumpProcess.stdout, outputStream);
      }
      
      // Wait for pg_dump process to complete
...
async function cleanupOldBackups(outputDir, retainDays) {
  ...
  try {
    ...
    // Strict backup file pattern to avoid deleting unrelated files
...
ENVIRONMENT VARIABLES:
  DATABASE_URL         PostgreSQL connection string (required)
  BACKUP_OUTPUT_DIR    Default backup output directory
  BACKUP_RETAIN_DAYS   Default retention period in days
...
EXAMPLES:
  ...
  node server/scripts/backup-database.js --compress --retain-days 90
